...
ORACLE_BASE="${ORACLE_BASE:-/data/oracle}"
INSTALLER_PATH="${INSTALLER_PATH:-/data/software}"
INSTALLER_FILE="${INSTALLER_FILE:-em13500_linux64.bin}"
RESPONSE_FILE="${RESPONSE_FILE:-/data/software/upgrade.rsp}"
...
if [ ! -f "$RESPONSE_FILE" ]; then
    ...
    print_info "Using response file: $RESPONSE_FILE"
        ##unset CLASSPATH
	##RESPONSE_FILE_OPTION="EMPREREQ_KIT=true -silent -responseFile $RESPONSE_FILE -invPtrLoc /data/oracle/oraInventory/oraInst.loc"
	RESPONSE_FILE_OPTION=" -silent -responseFile $RESPONSE_FILE -invPtrLoc /data/oracle/oraInventory/oraInst.loc -J-Djava.io.tmpdir=/data/tmp/"
fi
...
if [ -n "$RESPONSE_FILE_OPTION" ]; then
    ...
    ./"${INSTALLER_FILE}" ${RESPONSE_FILE_OPTION} -invPtrLoc /data/oracle/oraInventory/oraInst.loc
else
